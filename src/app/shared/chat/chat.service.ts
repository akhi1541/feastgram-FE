...
export class ChatService {
  ...
  private url = 'http://localhost:3003'; // Ensure this matches your server URL
  ...
  fetchMessages(receiverId: string, senderId: string): Observable<any> {
    return this.http.get<any>(
      `${this.contextRoute}api/v1/chats/messages/${receiverId}/${senderId}`
    );
  }
  ...
  getCommunicatedPeople(senderId: string) {
    return this.http.get(
      `${this.contextRoute}api/v1/chats/messages/${senderId}`
    );
  }
}
